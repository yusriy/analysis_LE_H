...
# Extract the msdwswrf variable (surface shortwave downward radiation in J/m^2)
msdwswrf <- ncvar_get(nc_file, "msdwswrf")

# Handle missing values
msdwswrf_fill_value <- -32767
msdwswrf[msdwswrf == msdwswrf_fill_value] <- NA

# Convert from J/m^2 to W/m^2 by dividing by 3600 (assuming hourly data)
msdwswrf <- msdwswrf / 3600
...
# Extract the radiation time series for the nine locations
msdwswrf_series_1 <- msdwswrf[lon_idx_1, lat_idx_1, ]
msdwswrf_series_2 <- msdwswrf[lon_idx_2, lat_idx_2, ]
msdwswrf_series_3 <- msdwswrf[lon_idx_3, lat_idx_3, ]
msdwswrf_series_4 <- msdwswrf[lon_idx_4, lat_idx_4, ]
msdwswrf_series_5 <- msdwswrf[lon_idx_5, lat_idx_5, ]
msdwswrf_series_6 <- msdwswrf[lon_idx_6, lat_idx_6, ]
msdwswrf_series_7 <- msdwswrf[lon_idx_7, lat_idx_7, ]
msdwswrf_series_8 <- msdwswrf[lon_idx_8, lat_idx_8, ]
msdwswrf_series_9 <- msdwswrf[lon_idx_9, lat_idx_9, ]
...
msdwswrf_df_1 <- data.frame(time = time_myt, radiation = msdwswrf_series_1, location = locations[1])
msdwswrf_df_2 <- data.frame(time = time_myt, radiation = msdwswrf_series_2, location = locations[2])
msdwswrf_df_3 <- data.frame(time = time_myt, radiation = msdwswrf_series_3, location = locations[3])
msdwswrf_df_4 <- data.frame(time = time_myt, radiation = msdwswrf_series_4, location = locations[4])
msdwswrf_df_5 <- data.frame(time = time_myt, radiation = msdwswrf_series_5, location = locations[5])
msdwswrf_df_6 <- data.frame(time = time_myt, radiation = msdwswrf_series_6, location = locations[6])
msdwswrf_df_7 <- data.frame(time = time_myt, radiation = msdwswrf_series_7, location = locations[7])
msdwswrf_df_8 <- data.frame(time = time_myt, radiation = msdwswrf_series_8, location = locations[8])
msdwswrf_df_9 <- data.frame(time = time_myt, radiation = msdwswrf_series_9, location = locations[9])

# Combine data frames for all locations
msdwswrf_df <- bind_rows(msdwswrf_df_1, msdwswrf_df_2, msdwswrf_df_3, msdwswrf_df_4, msdwswrf_df_5, 
                     msdwswrf_df_6, msdwswrf_df_7, msdwswrf_df_8, msdwswrf_df_9)

# Calculate daily averages for all locations
daily_avg_msdwswrf_df <- msdwswrf_df %>%
  mutate(date = as.Date(time, tz = "Asia/Kuala_Lumpur")) %>%
  group_by(location, date) %>%
  summarize(radiation_avg = mean(radiation, na.rm = TRUE))
...
# Merge eddy covariance data (RG_1_1_1) with ERA5 radiation for comparison
combined_df <- left_join(daily_avg_msdwswrf_df, df_merged_day %>% select(date, RG_1_1_1), by = "date")
...
for (i in 1:9) {
  ...
  cor_test <- cor.test(df_location$RG_1_1_1, df_location$radiation_avg, use = "complete.obs")
  ...
  individual_scatter_plots[[i]] <- ggplot(df_location, aes(x = RG_1_1_1, y = radiation_avg)) +
    geom_point(size = 2, color = "blue") +  # Scatter plot for ERA5 radiation vs Eddy radiation
    geom_smooth(method = "lm", color = "black", se = FALSE, size = 0.8) +  # Linear regression trend line
    labs(x = if (show_x_label) expression(R[G][EC] ~ (W ~ m^-2)) else NULL,
         y = expression(R[G][ERA5] ~ (W ~ m^-2))) +  # Axis labels
    theme_minimal() +
    ...
    annotate("text", x = max(df_location$RG_1_1_1, na.rm = TRUE) * 0.8, y = max(df_location$radiation_avg, na.rm = TRUE) * 0.9, 
...
ggsave(filename = paste0(output_folder, "/combined_scatter_panel_radiation_lm_r_p_rearranged.jpeg"), 
