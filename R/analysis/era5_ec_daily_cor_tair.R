...
# Extract the t2m variable (air temperature at 2m in Kelvin)
t2m <- ncvar_get(nc_file, "t2m")

# Handle missing values
t2m_fill_value <- -32767
t2m[t2m == t2m_fill_value] <- NA
...
t2m_series_1 <- t2m[lon_idx_1, lat_idx_1, ]
t2m_series_2 <- t2m[lon_idx_2, lat_idx_2, ]
t2m_series_3 <- t2m[lon_idx_3, lat_idx_3, ]
t2m_series_4 <- t2m[lon_idx_4, lat_idx_4, ]
t2m_series_5 <- t2m[lon_idx_5, lat_idx_5, ]
t2m_series_6 <- t2m[lon_idx_6, lat_idx_6, ]
t2m_series_7 <- t2m[lon_idx_7, lat_idx_7, ]
t2m_series_8 <- t2m[lon_idx_8, lat_idx_8, ]
t2m_series_9 <- t2m[lon_idx_9, lat_idx_9, ]
...
t2m_df_1 <- data.frame(time = time_myt, air_temperature = t2m_series_1, location = locations[1])
t2m_df_2 <- data.frame(time = time_myt, air_temperature = t2m_series_2, location = locations[2])
t2m_df_3 <- data.frame(time = time_myt, air_temperature = t2m_series_3, location = locations[3])
t2m_df_4 <- data.frame(time = time_myt, air_temperature = t2m_series_4, location = locations[4])
t2m_df_5 <- data.frame(time = time_myt, air_temperature = t2m_series_5, location = locations[5])
t2m_df_6 <- data.frame(time = time_myt, air_temperature = t2m_series_6, location = locations[6])
t2m_df_7 <- data.frame(time = time_myt, air_temperature = t2m_series_7, location = locations[7])
t2m_df_8 <- data.frame(time = time_myt, air_temperature = t2m_series_8, location = locations[8])
t2m_df_9 <- data.frame(time = time_myt, air_temperature = t2m_series_9, location = locations[9])

# Combine data frames for all locations
t2m_df <- bind_rows(t2m_df_1, t2m_df_2, t2m_df_3, t2m_df_4, t2m_df_5, 
                    t2m_df_6, t2m_df_7, t2m_df_8, t2m_df_9)

# Calculate daily averages for all locations
daily_avg_t2m_df <- t2m_df %>%
  mutate(date = as.Date(time, tz = "Asia/Kuala_Lumpur")) %>%
  group_by(location, date) %>%
  summarize(air_temperature_avg = mean(air_temperature, na.rm = TRUE))
...
# Merge eddy covariance data (TA_1_1_1) with ERA5 air temperature for comparison
combined_df <- left_join(daily_avg_t2m_df, df_merged_day %>% select(date, TA_1_1_1), by = "date")
...
for (i in 1:9) {
  ...
  cor_test <- cor.test(df_location$TA_1_1_1, df_location$air_temperature_avg, use = "complete.obs")
  ...
  individual_scatter_plots[[i]] <- ggplot(df_location, aes(x = TA_1_1_1, y = air_temperature_avg)) +
    ...
    labs(x = if (show_x_label) expression(T[EC] ~ (degree * C)) else NULL,
         y = expression(T[ERA5] ~ (degree * C))) +  # Axis labels
    theme_minimal() +
    ...
    annotate("text", x = max(df_location$TA_1_1_1, na.rm = TRUE) * 0.8, y = max(df_location$air_temperature_avg, na.rm = TRUE) * 0.9, 
...
ggsave(filename = paste0(output_folder, "/combined_scatter_panel_air_temperature_lm_r_p_rearranged.jpeg"), 
