...
for (i in 1:nrow(locations)) {
  ...
  # Separate sensible heat from eddy covariance (H_ec) and ERA5 (H_era5)
  ...
  # Calculate the bias (mean difference between H_ec and H_era5)
  bias <- mean(H_era5, na.rm = TRUE) - mean(H_ec, na.rm = TRUE)
  
  # Apply linear scaling to correct H_era5 by subtracting the bias
  H_era5_corrected <- H_era5 - bias
