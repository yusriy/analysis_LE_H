...
for (i in 1:nrow(locations)) {
  ...
  # Ensure that both LE_ec and LE_era5_shifted have the same length
  if (length(merged_data_shifted$LE.x) != length(merged_data_shifted$LE_era5_shifted)) {
    warning(paste("Length mismatch in EC and ERA5 data at location:", locations$name[i]))
    next
  }
  ...
  # Ensure that LE_ec and LE_era5_qmapped have the same length
  if (length(merged_data_shifted$LE.x) != length(LE_era5_qmapped)) {
    warning(paste("Length mismatch in EC and Quantile Mapped ERA5 data at location:", locations$name[i]))
    next
  }
  ...
  # Print metrics
  print(paste("Point:", locations$name[i]))
  print(paste("Correlation after Quantile Mapping:", round(correlation_qmapped, 3)))
  print(paste("RMSE after Quantile Mapping:", round(rmse_qmapped, 3)))
}